...
st.set_page_config(
    page_title="Drowsiness Detection | 졸음감지",
    page_icon="🥱😪😴",
...
with col1:
    st.title("Drowsiness Detection!!! | 졸음 감지!!!🥱😪😴")
    with st.container():
        c1, c2 = st.columns(spec=[1, 1])
        with c1:
            # The amount of time (in seconds) to wait before sounding the alarm.
            WAIT_TIME = st.slider("Seconds to wait before sounding alarm | 알람이 울리기 전에 대기하는 시간(초):", 0.0, 5.0, 1.0, 0.25)

        with c2:
            # Lowest valid value of Eye Aspect Ratio. Ideal values [0.15, 0.2].
            EAR_THRESH = st.slider("Eye Aspect Ratio threshold | 눈 종횡비 임계값:", 0.0, 0.4, 0.18, 0.01)
